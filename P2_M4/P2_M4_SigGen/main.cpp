/***************************************************************************************************
    ...
    @brief: 1. Generates a ramp waveform @ 1004 Hz (Sodtware DAC)
            2. Detects frequency of input ADC signal using peak detection algorithm
            3. Blinks the on-board LED at a rate proportional to the ADC input frequency
...
int main(void){
   ...
   // Calculating maximum value of input signal
   ...
   // Calculating thresholds for peak detection 
   thresh_max = 0.75 * max;
   thresh_min = 10;

   time_t time_temp = clock();

   // Calculating peaks in 1 sec (frequency)
   while((start = clock() - time_temp) < 101){
    ...
    while(1){
        ...
        dout_1 = !dout_1;       // Toggle D7 pin 
        wait_ms(pulse_count);   // Wait for pulse count period
    } 
